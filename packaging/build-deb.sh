...
case "$1" in
    configure)
        ...
        # TLS cert is generated by libpam-web3-signing-host.service (which
        # also resolves the signing host) so that the cert's SAN matches
        # whatever hostname/sslip.io address the user actually sees in the
        # signing URL. Just prep the directory here.
        mkdir -p /etc/libpam-web3/tls
        chmod 755 /etc/libpam-web3/tls
